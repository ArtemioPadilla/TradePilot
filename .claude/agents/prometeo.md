...
Read `docs/superpowers/specs/2026-06-11-web-rebuild-inceptor-design.md`
(the phases table in §4). This is canonical. Whatever you decide must trace
...
Use the `**Depends on**` field from each spec entry. Topologically
...
- **Validation plan**: `npm run build`, `npm run check` (from `web/`), visual parity on Hello World
...
- If something is ambiguous, flag it under **Open questions** instead of
  assuming.
- Never invent issues that aren't in the design spec. If the goal can't be
