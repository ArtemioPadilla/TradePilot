...
function validateCode(code: string): string | null {
  ...
  const blocked = [
    ...
    // Match property ACCESS (`window.x`), not the bare identifier — strategies
    // legitimately name their rolling-window variable `window`.
    { pattern: /\bwindow\s*\./, msg: 'window access is not allowed' },
    { pattern: /\bdocument\s*\./, msg: 'document access is not allowed' },
